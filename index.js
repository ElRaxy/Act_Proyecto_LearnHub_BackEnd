...
const corsOptions = {
    origin: (origin, callback) => {
        ...
        if (whiteList.includes(origin) || !origin) {
            ...
            // Usamos AppError para que tu middleware de errores lo capture
            callback(new AppError("No permitido por CORS - No pasarás!", 403));
        }
    },
    credentials: true // Permite que el JWT en cookies viaje entre puertos
};
...
app.use((err, req, res, next) => {
  ...
  logger.error.error(`❌ ERROR: ${err.message} | URL: ${req.originalUrl} | Method: ${req.method}`);
...
setInterval(() => {
  ...
  console.log(`\n${colorBannerLine("╔═══════════════════════════════════════════════════╗")}`);
  console.log(`${colorBannerLine(`║                Consola Actualizada                ║`)}`);
  console.log(`${colorBannerLine(`║  Hora: ${now}                                   ║`)}`);
  console.log(`${colorBannerLine("╚═══════════════════════════════════════════════════╝")}\n`);
}, 50000);