  ...
  definition: {
    ...
    components: {
      schemas: {
        Course: {
          type: 'object',
          required: ['title', 'description', 'category', 'startDate', 'endDate', 'location'],
          properties: {
            _id: { type: 'string', description: 'ID único del curso', example: '507f1f77bcf86cd799439011' },
            title: { type: 'string', description: 'Título del curso', example: 'Introducción a JavaScript' },
            description: { type: 'string', description: 'Descripción del curso', example: 'Curso completo de JavaScript desde cero' },
            category: { type: 'string', description: 'Categoría del curso', example: 'Programación' },
            startDate: { type: 'string', format: 'date-time', description: 'Fecha de inicio del curso', example: '2024-01-15T09:00:00.000Z' },
            endDate: { type: 'string', format: 'date-time', description: 'Fecha de fin del curso', example: '2024-03-15T18:00:00.000Z' },
            location: { type: 'string', description: 'Ubicación del curso', example: 'Aula 101' },
        ...
        User: {
          ...
          properties: {
            ...
            email: { type: 'string', format: 'email', description: 'Email del usuario', example: 'juan.perez@example.com' },
            phone: { type: 'string', description: 'Teléfono del usuario', example: '612345678' },
            birthDate: { type: 'string', format: 'date', description: 'Fecha de nacimiento', example: '1990-05-15' },
            profile: { type: 'string', enum: ['ADMINISTRADOR', 'PROFESOR', 'ALUMNO'], description: 'Perfil del usuario', example: 'ALUMNO' },
            createdAt: { type: 'string', format: 'date-time', description: 'Fecha de creación' },
            updatedAt: { type: 'string', format: 'date-time', description: 'Fecha de última actualización' }
          }
        },
        Enrollment: {
          ...
          properties: {
            ...
            notes: { type: 'string', description: 'Notas adicionales', example: 'Estudiante interesado en el curso' },
            createdAt: { type: 'string', format: 'date-time', description: 'Fecha de creación' },
            updatedAt: { type: 'string', format: 'date-time', description: 'Fecha de última actualización' }
          }
        },
        Error: {
          type: 'object',
          properties: {
            error: { type: 'string', description: 'Mensaje de error', example: 'Error obteniendo cursos: Curso no encontrado' }
          }
        }
      }
    }
